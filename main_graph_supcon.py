...
class Classifier(torch.nn.Sequential):
    def __init__(self, model_1, model_2, opt):
        ...
        self.fusion.add_module('fusion_layer_1', torch.nn.Linear(in_features=dim_feat*4, out_features=dim_feat*2))
        ...
        self.fusion.add_module('fusion_layer_3', torch.nn.Linear(in_features=dim_feat*2, out_features=opt.num_tasks))

        self.fusion_global = torch.nn.Sequential()
        self.fusion_global.add_module('fusion_layer_1', torch.nn.Linear(in_features=dim_feat*4+2048+167, out_features=dim_feat*2))
    ...
    def forward(self, batch1, opt, phase='train'):
        ...
        f2_raw = self.model_2(batch1.input_ids.view(batch1.y.shape[0],-1).int(), batch1.attention_mask.view(batch1.y.shape[0],-1).int())
...
def set_model(opt):
    model_1 = SupConDeeperGCN(num_tasks=opt.num_tasks,mlp_layers=opt.mlp_layers,num_gc_layers=opt.num_gc_layers)
    ...
    if opt.classification:
        ...
        criterion_scl = SupConLoss(temperature=opt.temp, base_temperature=opt.temp,gamma1=opt.gamma1,gamma2=opt.gamma2, threshold=opt.threshold)
...
def train(train_dataset, model, criterion_scl, criterion_mse, criterion_task, optimizer, epoch, opt, mu=0, std=0):
    """one epoch training"""
    model.train()
...
def validation(val_dataset, model, optimizer, epoch, opt, mu=0, std=0, save_feature=0):
    ...
    with torch.no_grad():
        ...
        if opt.num_tasks>1:
            ...
            input_dict = {"y_true": np.expand_dims(y_true.squeeze(),1), "y_pred": np.expand_dims(y_pred.squeeze(),1)}
    ...
    if save_feature:
        ...
        return auroc, feature_smiles, feature_graph, y_true, y_pred, feature_smiles_sp, feature_graph_sp, feature
    else:
        return auroc
...
def main():
    ...
    for dn in [opt.dataset+'_1',opt.dataset+'_2',opt.dataset+'_3']:
        ...
        for epoch in range(1, opt.epochs + 1):
            ...
            # train for one epoch
            time1 = time.time()
            loss_task, loss_recon, loss_scl, loss = train(train_dataset, model, criterion_scl, criterion_mse, criterion_task, optimizer, epoch, opt, mu, std)
            time2 = time.time()
        ...
        test_acc, feature_smiles, feature_graph, y_true, y_pred, feature_smiles_sp, feature_graph_sp, feature = validation(test_dataset, best_model.cuda(), optimizer, epoch, opt, mu, std, save_feature=1)
