...
class SupConLoss(nn.Module):
    ...
    def __init__(self, temperature=0.07, contrast_mode='one', base_temperature=0.07, gamma1=2, gamma2=2, threshold=0.5):
    ...
    def forward(self, features, labels=None, mask=None):
        ...
        anchor_dot_contrast = torch.div(
            torch.matmul(anchor_feature, contrast_feature_smiles.T),
            self.temperature)
        ...
        log_prob = torch.log(torch.exp(logits*n_weight*mask)) -  torch.log(exp_logits.sum(1, keepdim=True))
        ...
        anchor_dot_contrast = torch.div(
            torch.matmul(anchor_feature, contrast_feature_graph.T),
            self.temperature)
        # for numerical stability
        logits_max, _ = torch.max(anchor_dot_contrast, dim=1, keepdim=True)
        logits = anchor_dot_contrast - logits_max.detach()
