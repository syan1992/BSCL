...
from deepgcn_nn import AtomEncoder, BondEncoder, MLP, norm_layer
...
class SupConDeeperGCN(torch.nn.Module):
    ...
    def __init__(self, head='mlp', mlp_layers=2, feat_dim=128, num_tasks=1,num_gc_layers=3):
        ...
        mlp_layers = mlp_layers
        norm = 'layer'
        self.encoder = DeeperGCN(dim, num_gc_layers, dropout, block, conv_encode_edge, add_virtual_node, hidden_channels, self.num_tasks, aggr, t=t, learn_t = learn_t, mlp_layers=mlp_layers, norm=norm)

        dim_in = hidden_channels*2
